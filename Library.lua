...
function Library:ApplyCorner(Inst)
    ...
    if not (Inst:IsA('Frame') or Inst:IsA('TextButton') or Inst:IsA('TextBox') or Inst:IsA('ScrollingFrame')) then
        return;
    end;
    ...
    if Inst.Size.Y.Scale == 0 and Inst.Size.Y.Offset > 0 and Inst.Size.Y.Offset <= 3 then
        return;
    end;

    if Inst.Size.X.Scale == 0 and Inst.Size.X.Offset > 0 and Inst.Size.X.Offset <= 3 then
        return;
    end;

    if Inst:FindFirstChild('LinoriaCorner') then
        return;
    end;
...
function Library:AddGlow(Inst, Thickness, Transparency)
    local Stroke = Library:Create('UIStroke', {
        ...
        Transparency = Transparency or Library.GlowTransparency;
        Parent = Inst;
    });
...
function Library:CreateWindow(...)
    ...
    if InnerCorner then
        InnerCorner.CornerRadius = Library.WindowCornerRadius;
    end;
    ...
    Library:AddToRegistry(IconHolder, {
        BackgroundColor3 = 'BackgroundColor';
        BorderColor3 = 'OutlineColor';
    });
    ...
    function Window:AddTab(Name)
        ...
        function Tab:ShowTab()
            ...
            TweenService:Create(LeftSide, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                CanvasPosition = Vector2.new(0, 0);
            }):Play();

            TweenService:Create(RightSide, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                CanvasPosition = Vector2.new(0, 0);
            }):Play();
        end;
